...
final class Recipient extends Model
{
    ...
    {
        $sql = 'INSERT INTO recipients (email, first_name, last_name, institution, country, segment, status, consent_at) VALUES (:email, :first_name, :last_name, :institution, :country, :segment, :status, :consent_at)';
        $this->execute($sql, [
            'email' => $data['email'],
    ...
    {
        $sql = 'UPDATE recipients SET email = :email, first_name = :first_name, last_name = :last_name, institution = :institution, country = :country, segment = :segment, status = :status, consent_at = :consent_at WHERE id = :id';
        $this->execute($sql, [
            'id' => $id,
            'email' => $data['email'],
            'first_name' => $data['first_name'] ?: null,
    ...
    {
        $existing = $this->fetchOne('SELECT id FROM recipients WHERE email = :email LIMIT 1', ['email' => $data['email']]);
        if ($existing) {
    ...
    public function generateOrGetToken(int $recipientId): string
    {
        ...
        $this->execute('INSERT INTO unsubscribe_tokens (recipient_id, token) VALUES (:recipient_id, :token)', [
    ...
    public function findByUnsubscribeToken(string $token): ?array
    {
        return $this->fetchOne(
            'SELECT r.*, ut.id AS token_id, ut.used_at FROM unsubscribe_tokens ut INNER JOIN recipients r ON r.id = ut.recipient_id WHERE ut.token = :token LIMIT 1',
            ['token' => $token]
        );
    }
    ...
    {
        $this->execute('UPDATE recipients r INNER JOIN unsubscribe_tokens ut ON ut.recipient_id = r.id SET r.unsubscribed_at = NOW(), r.status = "unsubscribed", ut.used_at = IFNULL(ut.used_at, NOW()) WHERE ut.token = :token', ['token' => $token]);
    }

    public function createImportRecord(int $userId, string $filename, int $total, int $imported, int $failed): void
    {
        $this->execute('INSERT INTO recipient_imports (uploaded_by, original_filename, total_rows, imported_rows, failed_rows) VALUES (:uploaded_by, :filename, :total_rows, :imported_rows, :failed_rows)', [
            'uploaded_by' => $userId,
            'filename' => $filename,
            'total_rows' => $total,
