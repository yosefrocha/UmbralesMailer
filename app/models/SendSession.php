...
final class SendSession extends Model
{
    ...
    {
        $this->execute(
            'INSERT INTO send_sessions (campaign_id, campaign_message_id, status, started_at)
             VALUES (:campaign_id, :campaign_message_id, "queued", NOW())',
            [
    ...
    {
        $sql = 'SELECT ss.*, c.name AS campaign_name
                FROM send_sessions ss
    ...
    public function latestByCampaign(int $campaignId): ?array
    {
        return $this->fetchOne(
            'SELECT * FROM send_sessions WHERE campaign_id = :campaign_id ORDER BY id DESC LIMIT 1',
            ['campaign_id' => $campaignId]
        );
    }

    public function addItemsFromRecipients(int $sessionId, array $recipients): void
    {
        foreach ($recipients as $recipient) {
            $this->execute(
                'INSERT IGNORE INTO send_session_items (send_session_id, recipient_id)
                 VALUES (:send_session_id, :recipient_id)',
                [
    ...
    {
        $sql = 'SELECT ssi.*, r.email, r.status AS recipient_status, r.unsubscribed_at
                FROM send_session_items ssi
                INNER JOIN recipients r ON r.id = ssi.recipient_id
                WHERE ssi.send_session_id = :session_id
                  AND ssi.status = "pending"
                ORDER BY ssi.id ASC
                LIMIT ' . max(1, $limit);
    ...
    public function markItemSent(int $itemId, string $messageId): void
    {
        $this->execute(
            'UPDATE send_session_items
             SET ses_message_id = :ses_message_id,
                 status = "sent",
                 processed_at = NOW(),
                 updated_at = NOW()
             WHERE id = :id',
    ...
    public function refreshCounts(int $sessionId): array
    {
        $row = $this->fetchOne(
            'SELECT COUNT(*) AS total_count,
                    SUM(CASE WHEN status IN ("sent","failed","skipped") THEN 1 ELSE 0 END) AS processed_count,
                    SUM(CASE WHEN status = "sent" THEN 1 ELSE 0 END) AS success_count,
                    SUM(CASE WHEN status = "failed" THEN 1 ELSE 0 END) AS failed_count,
                    SUM(CASE WHEN status = "pending" THEN 1 ELSE 0 END) AS pending_count
             FROM send_session_items
